    ...
    def handle(self, *args, **options):
        ...
        if local_photo.exists():
            with open(local_photo, 'rb') as f:
                profile.photo.save('Seydna.jpeg', File(f), save=False)
        ...
        # Upload CV (resource_type=auto in settings detects PDF as raw)
        local_cv = media_root / 'cv' / 'seydna_aly_cv.pdf'
        if local_cv.exists():
            with open(local_cv, 'rb') as f:
                profile.cv_file.save('seydna_aly_cv.pdf', File(f), save=False)
